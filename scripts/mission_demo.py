...
from geometry_msgs.msg import PoseStamped, QuaternionStamped, TransformStamped
...
def arm(status):
    ...
    try:
        ...
        service_caller = rospy.ServiceProxy(
...
def takeoff(altitude):
    ...
    try:
        ...
        takeoff_request = cmd_takeoff(
            altitude=altitude, latitude=0, longitude=0, min_pitch=0, yaw=0
        )
        service_caller = rospy.ServiceProxy(
...
def move_to_position(x, y, z):
    ...
    if (x < local_state.x_lim) & (y < local_state.y_lim) & (z < local_state.z_lim):
        ...
        position_target_pub = rospy.Publisher(
            "/mavros/setpoint_raw/local", mavros_msgs.msg.PositionTarget, queue_size=10
        )
